...
// Hook for pages that should show the tone setup modal automatically after login
export const useToneSetupAutoShow = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      console.log('🔍 Verificando needs_tone_setup (auto-show):', { needsToneSetup, postponed, hasChecked });
...
// Hook for pages that should NOT show the modal automatically (only when user tries to generate)
export const useToneSetupOnDemand = () => {
  const [showToneModal, setShowToneModal] = useState(false);
